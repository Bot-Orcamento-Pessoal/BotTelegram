...
const voltarMenu = { reply_markup: { inline_keyboard: [ [{ text: '⬅️ Voltar ao menu', callback_data: 'voltar_menu' }] ] } };
...
bot.onText(//start/, (msg) => { bot.sendMessage(msg.chat.id, 'Escolha uma opção:', botoesPrincipais); });

bot.on('callback_query', (query) => { const chatId = query.message.chat.id; const tipo = query.data;

switch (tipo) { case 'incluir_saldo': bot.sendMessage(chatId, 'Digite a descrição e o valor (ex: Salário, 1500):'); bot.once('message', (msg) => { const partes = msg.text.split(',').map(p => p.trim()); const valor = parseFloat(partes.pop().replace(',', '.')); const nome = partes.join(' ').trim(); if (!isNaN(valor)) { data.saldo += valor; salvar(); bot.sendMessage(chatId, Saldo atualizado com sucesso!\n\n${resumo()}, voltarMenu); } else { bot.sendMessage(chatId, 'Valor inválido.', voltarMenu); } }); break;
...
case 'incluir_despesa':
  ...
  bot.once('message', (msg) => {
    ...
    if (nome && !isNaN(valor)) {
      ...
      salvar();
      bot.sendMessage(chatId, 'Despesa registrada com sucesso.', voltarMenu);
      bot.sendMessage(chatId, resumoDespesas(), voltarMenu);
    } else {
      bot.sendMessage(chatId, 'Formato inválido. Use: Nome, valor', voltarMenu);
    }
  });
  break;
...
case 'gasto_sodexo':
  ...
  bot.once('message', (msg) => {
    ...
    if (gastosRegistrados.length > 0) {
      ...
      bot.sendMessage(chatId, 'Nenhum gasto válido foi informado. Use: Nome, valor', voltarMenu);
    }
  });
  break;
...
case 'listar_gastos':
  if (data.gastos.length === 0) {
    bot.sendMessage(chatId, 'Nenhum gasto registrado.', voltarMenu);
  } else {
    const lista = data.gastos.map((g, i) => `${i + 1}. ${g.nome} - R$ ${g.valor.toFixed(2)} (${g.tipo})`).join('\n');
    bot.sendMessage(chatId, lista, voltarMenu);
  }
  break;
...
case 'pagar_despesa':
  ...
  if (pendentes.length === 0) {
    bot.sendMessage(chatId, 'Nenhuma despesa pendente.', voltarMenu);
  } else {
...
case 'voltar_menu':
  bot.sendMessage(chatId, 'Escolha uma opção:', botoesPrincipais);
  break;
...
default:
  if (tipo.startsWith('pagar_')) {
    ...
    if (!isNaN(index) && data.despesasFixas[index] && !data.despesasFixas[index].pago) {
      ...
      salvar();
      bot.sendMessage(chatId, `Despesa "${despesa.nome}" marcada como paga. Valor de R$ ${despesa.valor.toFixed(2)} subtraído do saldo.`, voltarMenu);
      bot.sendMessage(chatId, resumoDespesas(), voltarMenu);
      bot.sendMessage(chatId, resumo(), voltarMenu);
    } else {
      bot.sendMessage(chatId, 'Despesa inválida ou já paga.', voltarMenu);
    }
  }
  break;
