const TelegramBot = require('node-telegram-bot-api'); const fs = require('fs'); const token = 'SEU_TOKEN_AQUI'; // Substitua pelo seu token real
...
const resumo = () => { const totalGasto = data.gastos .filter(g => g.tipo !== 'cartão' && g.tipo !== 'sodexo') .reduce((sum, g) => sum + g.valor, 0); const gastosCartao = data.gastos .filter(g => g.tipo === 'cartão') .reduce((sum, g) => sum + g.valor, 0); const gastosSodexo = data.gastos .filter(g => g.tipo === 'sodexo') .reduce((sum, g) => sum + g.valor, 0);
...
switch (tipo) { case 'incluir_saldo': bot.sendMessage(chatId, 'Digite o valor a ser adicionado:'); bot.once('message', (msg) => { const valor = parseFloat(msg.text.replace(',', '.')); if (!isNaN(valor)) { data.saldo += valor; salvar(); bot.sendMessage(chatId, Saldo atualizado!\n\n${resumo()}); } else { bot.sendMessage(chatId, 'Valor inválido.'); } }); break;
